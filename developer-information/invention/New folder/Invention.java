...
import com.rs.game.entity.Animation;
import com.rs.game.entity.Graphics;
...
public class Invention extends Action {
	...
	public enum InventionAction {
		...
		CHARGE_PACK(1, 1, 1, new Item[] { new Item(36719) }, new Item(36719), Skills.INVENTION);
		...
		private InventionAction(int levelRequired, double experience, int emote, Item[] itemsRequired,
				Item producedItem, int skillType) {
	...
	public boolean process(Player player) {
		...
		if (!player.getInventory().containsItemToolBelt(invention.getItemsRequired()[0].getId(),
				invention.getItemsRequired()[0].getAmount())) {
			return false;
		}
		if (invention.getItemsRequired().length > 1) {
			if (!player.getInventory().containsItemToolBelt(invention.getItemsRequired()[1].getId(),
					invention.getItemsRequired()[1].getAmount())) {
				return false;
			}
		}
		if (!player.getSkills().hasLevel(invention.getSkillType(), invention.getLevelRequired()))
			return false;
		return true;
	}

	public static InventionAction isMixing(Player player, Item used, Item usedWith) {
	...
	public int processWithDelay(Player player) {
		...
		player.getInventory().addItem(invention.getProducedItem().getId(), amount);
